...
def operationCreate() {
    script {
        ...
        if(dirExistingResult == '1'){
            ...
            sh "set +x && cd provision && terraform init ../../${mainDirectoryName}/${params.Login}/${params.MachineName} && ./createcontrail \"--create\" \"${params.Login}\" \"${params.Password}\" \"${params.MachineName}\" \"${params.ProjectID}\" \"${params.domainName}\" \"${params.projectName}\" \"${params.networkName}\" \"../../${mainDirectoryName}/${params.Login}/${params.MachineName}/${params.Login}-key.pub\" \"../../${mainDirectoryName}/${params.Login}/${params.MachineName}/${params.Login}-key.priv\" \"${params.routerIP}\" \"${params.orchestrator}\" \"${params.branch}\" \"${params.flavor}\" \"${terraformStateFileName}\" && set -x"
        }
    }
}
...
def operationDestroy() {
    script {
        ...
        sh "set +x && cd provision && terraform init ../../${mainDirectoryName}/${params.Login}/${params.MachineName} && ./createcontrail \"--destroy\" \"${params.Login}\" \"${params.Password}\" \"${params.MachineName}\" \"${params.ProjectID}\" \"${params.domainName}\" \"${params.projectName}\" \"${params.orchestrator}\" \"${terraformStateFileName}\" && set -x"
...
def operationListMachines() {
    script {
        lock('shared files area') {
            ...
            dirExistingResult = checkIfDirectoryExists("../${mainDirectoryName}/${params.Login}")
            if ("${params.Login}" == "") {
                errorEcho("You need to specify user login first!", wrongParametersError)
            } else if (dirExistingResult == '0') {
...
pipeline {
    ...
    parameters {
        ...
        choice(choices: ['--create', '--destroy', '--list-resources', '--list-machines', '--list-users', '--refresh-workspace', '--delete-user-dir', '--delete-machine-dir'], description: '', name: 'Operation')
